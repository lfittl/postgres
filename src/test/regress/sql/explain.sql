...
BEGIN
    ...
    IF workers IS NOT NULL THEN
        FOR i IN 0..json_array_length(workers) - 1 LOOP
            total_exact_pages := total_exact_pages +
                COALESCE((workers->i->>'Exact Heap Blocks')::int, 0);
        END LOOP;
    END IF;

    RETURN total_exact_pages > 0;
END;
$$ LANGUAGE plpgsql;
