...
typedef struct RecursiveUnion
{
	...
	Cardinality numGroups pg_node_attr(query_jumble_ignore);
} RecursiveUnion;
...
typedef struct Memoize
{
	...
	Cardinality est_calls pg_node_attr(query_jumble_ignore);

	/* Estimated number of distinct lookup keys, for EXPLAIN */
	Cardinality est_unique_keys pg_node_attr(query_jumble_ignore);

	/* Estimated cache hit ratio, for EXPLAIN */
	double		est_hit_ratio pg_node_attr(query_jumble_ignore);
...
typedef struct WindowAgg
{
	...
	char	   *winname pg_node_attr(query_jumble_ignore);
