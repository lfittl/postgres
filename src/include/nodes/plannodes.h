...
typedef struct Plan
{
	 ...
	 */
	Cost		startup_cost pg_node_attr(query_jumble_ignore);	/* cost expended before fetching any tuples */
	Cost		total_cost pg_node_attr(query_jumble_ignore); /* total cost (assuming all tuples fetched) */

	/*
	 * planner's estimate of result size of this plan step
	 */
	Cardinality plan_rows pg_node_attr(query_jumble_ignore);		/* number of rows plan is expected to emit */
	int			plan_width pg_node_attr(query_jumble_ignore);		/* average row width in bytes */
	...
	List	   *initPlan pg_node_attr(query_jumble_ignore);		/* Init Plan nodes (un-correlated expr
								 * subselects) */
...
typedef struct ModifyTable
{
	...
	List	   *fdwPrivLists pg_node_attr(query_jumble_ignore);	/* per-target-table FDW private data lists */
...
typedef struct CustomScan
{
	...
	List	   *custom_private pg_node_attr(query_jumble_ignore); /* private data for custom code */
...
typedef struct Hash
{
	...
	Cardinality rows_total pg_node_attr(query_jumble_ignore);		/* estimate total rows if parallel_aware */
} Hash;
