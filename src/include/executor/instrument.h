...
typedef struct Instrumentation
{
	...
	/*
	 * Tracking of instrumentation entries that need to be cleaned up on abort
	 * that are not registered as a resource owner themselves (i.e. per-node
 ...
 *
 * When the stack is empty, 'current' points to TopInstrumentation which
 * accumulates session-level totals.
 */
typedef struct InstrStackState
{
	...
	Instrumentation *current;	/* top of stack, or &TopInstrumentation when
								 * empty */
} InstrStackState;
 ...
 */
extern PGDLLIMPORT Instrumentation TopInstrumentation;
...
extern void InstrPopAndFinalizeStack(Instrumentation *instr);
...
extern Instrumentation *InstrStop(Instrumentation *instr, bool finalize);
...
extern void InstrRememberNode(Instrumentation *parent, Instrumentation *instr);
