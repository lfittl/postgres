 ...
 * via explicit callbacks from the transaction abort path.
 */
typedef struct Instrumentation
{
	...
	/* Optional link for abort handling in parent QueryInstrumentation */
 ...
 *
 * Abort cleanup is handled via AtAbort_Instrumentation() /
 * AtSubAbort_Instrumentation() called from the transaction abort path,
 * before portal cleanup destroys the executor memory context. In the case
 * of a transaction abort, logic equivalent to InstrQueryStopFinalize will
...
typedef struct QueryInstrumentation
{
	...
	/* Transaction nesting level at InstrQueryStart time, for abort cleanup */
	int			nestingLevel;

	/* Link in the global active_query_instrumentations list */
	dlist_node	active_node;

	/*
	 * Child Instrumentation entries that need to be accumulated on abort. On
	 * normal completion, the executor walks and accumulates these itself;
	 * this list is only consulted during abort recovery.
	 */
	dlist_head	unfinalized_children;
 ...
 * (executor finish) or AtAbort_Instrumentation on abort.
 */
...
extern void InstrQueryRememberChild(QueryInstrumentation *parent, Instrumentation *child);
...
extern void AtAbort_Instrumentation(void);
extern void AtSubAbort_Instrumentation(int nestLevel);
