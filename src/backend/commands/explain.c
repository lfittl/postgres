						 ...
						 QueryEnvironment *queryEnv)
{
	...
	if (es->buffers)
		instr = InstrAlloc(INSTRUMENT_TIMER | INSTRUMENT_BUFFERS);
	else
		instr = InstrAlloc(INSTRUMENT_TIMER);
	...
	switch (nodeTag(plan))
	{
		case T_IndexScan:
			...
			if (es->buffers && planstate->instrument)
				show_buffer_usage(es, &((IndexScanState *) planstate)->iss_InstrumentTableStack->bufusage, "Table");
			break;
		case T_IndexOnlyScan:
